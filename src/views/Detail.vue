<template>
	<div class="detail">
		<van-nav-bar title="商品详情" left-text="返回" right-text="分享" left-arrow fixed @click-left="onClickLeft" @click-right="showShare = true">
			<template #right>
		...
		<div class="info_box">
			<div class="header-image"><img class="auto_img" :src="productDetail.large_img" alt="" /></div>
			<div class="Pro_content">
				<div class="pro_info">
					...
					<div class="pro_rule">
						<!-- 样式选择 -->
						<div class="rule_item fl" v-for="(item, index) in productDetail.rules" :key="index">
					...
					<div class="Quantity fl">
						...
						<div class="count_box"><van-stepper v-model="product.count" disable-input /></div>
					</div>

					<div class="desc_box fl">
						<div class="pro_desc">商品描述</div>
						<div class="desc_text">
							<div class="text_item" v-for="(item, index) in productDetail.desc" :key="index">{{ index + 1 }}.{{ item }}</div>
						</div>
					</div>
				</div>
			</div>
		...
		<van-goods-action>
			...
			/>
			<van-goods-action-icon icon="like" :color="isLike ? '#e74c3c' : '#323233'" :text="isLike ? '已收藏' : '收藏'" @click="toggleLike" />
			<van-goods-action-button type="warning" text="加入购物车" @click="addShopbag" />
