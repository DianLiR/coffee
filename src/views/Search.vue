<template>
	<div class="search">
		<van-nav-bar fixed left-text="返回" left-arrow @click-left="goState">
			<template #right>
				<div><van-search v-model="name" ref="search" placeholder="请输入搜索关键词" @search="onSearch" @blur="onSearch" @input="onSearch" /></div>
			</template>
		</van-nav-bar>
		<Bgbox>
			<div v-if="productData.length>0">
				<div v-for="(item, index) in productData" :key="index" class="proitem_s clearfix" @click="goDetail(item.pid)"><Proitem :item="item"></Proitem></div>
			</div>
		</Bgbox>
	</div>
</template>
